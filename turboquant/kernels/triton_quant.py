...
@torch.jit.script
def packed_3bit_dequant_torch(packed: torch.Tensor, scales: torch.Tensor, levels: torch.Tensor, head_dim: int) -> torch.Tensor:
...
def benchmark_triton_kernels(
    ...
    packed = torch.randint(0, 256, (batch, heads, seq_len, packed_n), dtype=torch.uint8, device=device)
