...
class SpeculativeKVDraft:
    ...
    def compress_draft(self, keys: torch.Tensor, values: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    def decompress_draft(self, k_bits, v_bits, k_norms, v_norms, shapes) -> Tuple[torch.Tensor, torch.Tensor]:
...
class SpeculativePrefillEngine:
    ...
    def __init__(self, tq_cache: TurboQuantKVCache, config: SpeculativePrefillConfig):
        ...
        self.draft_helper = SpeculativeKVDraft(
            head_dim=tq_cache.config.head_dim, 
            num_heads=tq_cache.config.num_heads
        )
    ...
    def register_prompt_draft(self, prompt_id: str, keys: torch.Tensor, values: torch.Tensor) -> None:
    ...
    def speculative_compress(
        ...
        if prompt_id and prompt_id in self.draft_storage:
            ...
            k_approx, v_approx = self.draft_helper.decompress_draft(
                draft['k_bits'], draft['v_bits'], 
                draft['k_norms'], draft['v_norms'], 
                draft['shapes']
            )
            
            # Verify against precise keys/values
            accepted_mask, acceptance_rate = self.verify_draft(
                k_approx.to(keys.device), v_approx.to(values.device), 
                keys, values
            )
