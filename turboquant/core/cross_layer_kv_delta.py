...
from turboquant.core.turboquant import TurboQuantConfig, TurboQuantKVCache, CacheEntry
from turboquant.core.qjl import QJLResidualCorrector, QJLConfig
...
class CrossLayerDeltaConfig:
    ...
    delta_bits: int = 1
    delta_sketch_dim: Optional[int] = None # None -> head_dim // 8
...
class LayerKVEntry:
    ...
    layer_idx: int
    is_anchor: bool
    entry: Optional[CacheEntry] = None # full anchor entry if is_anchor or used_delta=False
    anchor_layer_idx: Optional[int] = None
    delta_keys: Optional[torch.Tensor] = None
    delta_values: Optional[torch.Tensor] = None
    delta_key_norms: Optional[torch.Tensor] = None
    delta_value_norms: Optional[torch.Tensor] = None
...
class CrossLayerKVDeltaCache:
    ...
    def compress_layer(
        ...
        if cos_sim >= self.config.similarity_threshold:
            ...
            tq_entry = self.anchor_cache.compress(keys, values)
            entry = LayerKVEntry(layer_idx=layer_idx, is_anchor=False, entry=tq_entry, layer_similarity=cos_sim, used_delta=False)
    ...
    def compress_layer_streaming(
        ...
        keys: torch.Tensor,
        values: torch.Tensor,
        anchor_keys: Optional[torch.Tensor] = None,
        ...
        if self.is_anchor_layer(layer_idx):
            ...
            if anchor_keys is None or anchor_values is None:
                raise ValueError(f"Delta layer {layer_idx} requires anchor_keys/anchor_values in streaming mode.")
            
            cos_sim_k = F.cosine_similarity(
                keys.float().reshape(-1, self.config.head_dim),
    ...
    def decompress_layer(self, layer_idx: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        k_delta = self.delta_corrector.decode(entry.delta_keys, entry.delta_key_norms, original_shape=k_pre.shape)
        v_delta = self.delta_corrector.decode(entry.delta_values, entry.delta_value_norms, original_shape=v_pre.shape)
        
        return (k_pre.float() + k_delta.float()).to(torch.float16), (v_pre.float() + v_delta.float()).to(torch.float16)
    ...
    def memory_usage_all(self) -> Dict[str, float]:
        ...
        for entry in self._layer_entries.values():
            ...
            if entry.is_anchor:
                 ...
                 num_delta += 1
                 # 1-bit delta + norms
                 delta_bytes = (entry.delta_keys.nbytes + entry.delta_values.nbytes + entry.delta_key_norms.nbytes + entry.delta_value_norms.nbytes)
