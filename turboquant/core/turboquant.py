...
from typing import Any, cast, TYPE_CHECKING, List, Tuple
...
from turboquant.core.polar_quant import PolarQuantizer, PolarQuantConfig
...
class CacheEntry:
    ...
    def to(self, device: str | torch.device) -> CacheEntry:
        self.compressed_keys = (self.compressed_keys[0].to(device), self.compressed_keys[1].to(device))
        self.compressed_values = (self.compressed_values[0].to(device), self.compressed_values[1].to(device))
...
class TurboQuantKVCache:
    ...
    def compress_async(self, keys: torch.Tensor, values: torch.Tensor, stream: torch.cuda.Stream | None = None) -> CacheEntry:
    ...
    def decompress(self, entry: CacheEntry) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        with self._lock:
            ...
            if self.qjl is not None and entry.residual_keys is not None:
                k_err = self.qjl.decode(entry.residual_keys, entry.residual_norms_k, original_shape=shape)
                v_err = self.qjl.decode(entry.residual_values, entry.residual_norms_v, original_shape=shape)
    ...
    def quality_metrics(self, original_keys: torch.Tensor, original_values: torch.Tensor) -> dict[str, float]:
        ...
        k_cos = float(F.cosine_similarity(
            k_orig.reshape(-1, self.config.head_dim),
            k_hat_f.reshape(-1, self.config.head_dim)
        ).mean())
        
        v_cos = float(F.cosine_similarity(
            v_orig.reshape(-1, self.config.head_dim),
            v_hat_f.reshape(-1, self.config.head_dim)
        ).mean())
