...
class KVCacheWatermarker:
    ...
    def embed(self, entry: CacheEntry, sequence_id: str) -> CacheEntry:
        ...
        if new_rk is not None:
             bits_rk = self._generate_watermark_bits(new_rk.shape, sequence_id + "_rk").to(new_rk.device)
             new_rk = _embed_in_lsb(new_rk, bits_rk)
        if new_rv is not None:
             bits_rv = self._generate_watermark_bits(new_rv.shape, sequence_id + "_rv").to(new_rv.device)
    ...
    def quality_impact(self, original_entry: CacheEntry, watermarked_entry: CacheEntry) -> Dict[str, float]:
