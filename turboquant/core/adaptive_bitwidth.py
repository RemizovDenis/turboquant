...
class AdaptiveBitwidthQuantizer:
    ...
    def decompress(
        ...
        for packed, scales, mask_u8, bitwidth in compressed.components:
            ...
            if token_count == 0:
                continue
            shape = (batch, heads, token_count, head_dim)
            sub_k = self.quantizers[bitwidth].dequantize(packed[0], scales[0], shape)
            sub_v = self.quantizers[bitwidth].dequantize(packed[1], scales[1], shape)
            keys[:, :, mask, :] = sub_k
            values[:, :, mask, :] = sub_v
