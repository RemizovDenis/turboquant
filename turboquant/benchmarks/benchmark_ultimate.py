...
class UltimateBenchmark:
    ...
    def bench_compression(self) -> Dict[str, Any]:
        tq_cfg = TurboQuantConfig(head_dim=self.config.head_dim, num_heads=self.config.num_heads, bits=3, device=self.config.device)
        tq = TurboQuantKVCache(tq_cfg)
        
        rows = []
        for seq in self.config.seq_lens:
            k = torch.randn(self.config.batch_size, self.config.num_heads, seq, self.config.head_dim, device=self.device)
    ...
    def bench_cross_layer(self) -> Dict[str, Any]:
        ...
        seq = 1024
        anchor_k = torch.randn(1, self.config.num_heads, seq, self.config.head_dim, device=self.device)
        ...
        for l in range(1, 4):
            ...
            cache.compress_layer(l, noisy_k, noisy_v)
            
        stats = cache.memory_usage_all()
        return stats

    def bench_speculative(self) -> Dict[str, Any]:
        tq_cfg = TurboQuantConfig(head_dim=self.config.head_dim, num_heads=self.config.num_heads, device=self.config.device)
        tq = TurboQuantKVCache(tq_cfg)
    ...
    def bench_fusion(self) -> Dict[str, Any]:
        ...
        fusion = TemporalExpertFusion(num_experts=8, expert_hidden_dim=4096, expert_ffn_dim=14336, config=cfg)
